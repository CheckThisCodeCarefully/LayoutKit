...
class ButtonLayoutTests: XCTestCase {
    ...
    private func verifyButtonLayout(textTestCase: Text.TestCase, type: UIButtonType) {
        ...
        XCTAssertEqual(layout.arrangement().frame.size, button.intrinsicContentSize, "fontName:\(textTestCase.font?.fontName) title:'\(textTestCase.text)' fontSize:\(textTestCase.font?.pointSize) type:\(type.rawValue)")
    }
}
